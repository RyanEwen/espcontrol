...
- **TTS Announcements** — sends the configured entry or exit message to Home Assistant when Voice Services are enabled. Home Assistant can use the `esphome.alarm_delay_announcement` event as a TTS automation hook for the panel's Voice Media Player.
