...
You can also enable **Allow media cover art to override**. It uses the **Media Player** selected in [Media Cover Art](/features/media-cover-art), and when that media player is in the `playing` state, the timer keeps waiting instead of putting the screen to sleep.
...
Below the presence entity, use **Then** to choose whether the panel dims the screen, shows the clock, or turns the display off when nobody is detected. This uses the same options as Timer mode, including **Allow media cover art to override**.
