...
inline constexpr int kAgendaViewDays = 14;
...
inline void agenda_view_render_(const AgendaList &list) {
  ...
  std::size_t idx = 0;
  for (int32_t day = ui.today; day < ui.today + kAgendaViewDays; day++) {
    if (day != ui.today &&
    ...
    while (idx < entries.size() && entries[idx].when.day_number <= day) {
      ...
      if (entry.when.day_number == day ||
        ...
        agenda_event_box_(events_col, entry, ui.today, ui.use_12h,
                          ...
                          ui.title_font, ui.small_font);
        any = true;
      }
      idx++;
    }
    if (!any) agenda_view_empty_row_(events_col, two_line_h, ui.small_font);
  }
}
...
inline void agenda_view_open(const std::string &entities_csv,
  ...
  int ey = 0, em = 0, ed = 0;
  agenda_civil_from_days(today + kAgendaViewDays, &ey, &em, &ed);
  char end[24];
